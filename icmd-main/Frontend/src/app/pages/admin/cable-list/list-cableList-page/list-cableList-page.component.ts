...
import { ListCableListTableComponent } from "@c/cableList/list-cableList-table";
import { CableListTableColumns } from "@u/constants";
import { CableListSearchHelperService, CableListService } from "src/app/service/cable-list";

@Component({
  standalone: true,
  selector: "app-list-cableList-page",
  templateUrl: "./list-cableList-page.component.html",
  imports: [
    CommonModule,
    MatFormFieldModule,
    ReactiveFormsModule,
...
export class ListCableListPageComponent extends FormBaseComponent<SearchNonInstrumentFilterModel> {
  @ViewChild("importFileInput", { static: false }) importFileInput!: ElementRef;
  @ViewChild(ListNonInstrumentTableComponent)
  nonInstrumentTable: ListNonInstrumentTableComponent;
  private customFilters$: BehaviorSubject<CustomFieldSearchModel[]> =
    new BehaviorSubject([]);
  ...
  protected nonInstrumentListColumns = [
    ...nonInstrumentListTableColumns.filter((x) => x.key != "actions"),
  ];
  private selectedColumns: string[] = [];
  private columnFilterList: CustomFieldSearchModel[] = [];

  constructor(
    protected _nonInstrumentSearchHelperService: NonInstrumentSearchHelperService,
  ...
  protected defaultCustomFilter(
    isExport: boolean = false,
    columnFilterList: CustomFieldSearchModel[] = []
  ): void {
    ...
    filters.push({
      fieldName: "isExport",
      fieldValue: isExport ? "true" : "false",
      searchType: SearchType.Contains,
      isColumnFilter: false,
    });
  ...
  protected exportData(): void {
    ...
    this._nonInstrumentSearchHelperService
      ...
      .subscribe((model) => {
        const columnMapping = this.nonInstrumentListColumns
          .filter((x) => this.selectedColumns.includes(x.key))
          .reduce((acc, column) => {
            acc[column.key] = column.label;
            return acc;
          }, {});
        this._excelHelper.exportExcel(
          model.items ?? [],
          columnMapping,
          fileName
        );
  ...
  protected getProjectTagFieldName(projectId: string): void {
    if (projectId) {
      forkJoin([
        ...
        this._commonService.getMemoryCacheItem(
          listColumnMemoryCacheKey.nonInstrumentColumn
        ),
      ])
        ...
        .subscribe((res) => {
          ...
          this.nonInstrumentTable.tagFieldNames.forEach(
          ...
          if (selectedColumn != null && selectedColumn.length > 0) {
            ...
            this.nonInstrumentTable.displayedColumns = [
              ...this.selectedColumns,
              nonInstrumentListTableColumns[
                nonInstrumentListTableColumns.length - 1
              ].key,
            ];
          } else {
            this.selectedColumns = this.nonInstrumentListColumns.map(
              (x) => x.key
            );
          }
  ...
  protected async openColumnSelectorDialog() {
    const data =
      await this._columnSelectorDialogService.openColumnSelectorDialog(
        this.nonInstrumentListColumns,
        listColumnMemoryCacheKey.nonInstrumentColumn
      );
    let selectedColumn = nonInstrumentListTableColumns.map((x) => x.key);
    if (data.selectedColumns.length > 0) selectedColumn = data.selectedColumns;
  ...
  private tableColumnchanges() {
    ...
    combineLatest(
      this.nonInstrumentTable.columnFiltersList.map((x) => x.columnFilterModel$)
    )
      .pipe(takeUntil(this._destroy$))
      .subscribe((res) => {
        if (res && res.length > 0) {
  ...
  private getNonInstrumentsDropdownInfo(): void {
    if (this.projectId) {
      this._commonService
        .getNonInstrumentDropdownInfo(this.projectId)
        .pipe(takeUntil(this._destroy$))
        .subscribe((res) => {
  ...
  private autoCompleteValueChange(): void {
    this.tagFilteredOptions = this.setupFilteredOptions(
      "tag",
      this.nonInstrumentDropdownData?.tagList || []
    );
    this.plcNumberFilteredOptions = this.setupFilteredOptions(
      "plcNo",
      this.nonInstrumentDropdownData?.plcNumberList || []
    );
    this.equipmentCodeFilteredOptions = this.setupFilteredOptions(
      "equipmentCode",
      this.nonInstrumentDropdownData?.equipmentCodeList || []
    );
    this.locationFilteredOptions = this.setupFilteredOptions(
      "location",
      this.nonInstrumentDropdownData?.locationList || []
    );
  }

  private setupFilteredOptions(
    field: string,
    dataList: DropdownInfoDtoModel[]
  ): Observable<DropdownInfoDtoModel[]> {
  ...
  protected onFileSelected(event: any): void {
    ...
    this._nonInstrumentService
      .importNonInstruments(this.projectId, selectedFile)
      .pipe(takeUntil(this._destroy$))
      .subscribe({
        next: (res) => {
          if (res && res.isSucceeded) {
          ...
          if (res.records && res.records?.length > 0) {
            this._excelHelper.downloadImportResponseFile<[]>(
              "NonInstruments",
              res.records,
              res.headers,
              true
  ...
  protected importFileDownload() {
    ...
    const csvConfig = mkConfig({
      filename: "Sample_NonInstrumentList",
      columnHeaders: columnMapping,
      fieldSeparator: ",",
    });
