...
import { ViewCableListDtoModel } from "./list-cableList-table.model";
import { CableListTableColumns } from "@u/constants";

@Component({
    standalone: true,
    selector: "app-list-cableList-table",
    templateUrl: "./list-cableList-table.component.html",
    imports: [
        FormDefaultsModule,
        MatTableModule,
        MatSortModule,
...
export class ListCableListTableComponent {
    ...
    @Input() dataSource: MatTableDataSource<ViewCableListDtoModel>;
    @Input() totalLength: number = 0;
    @Input() tagFieldNames: string[] = [];

    public displayedColumns = [...CableListTableColumns].map(x => x.key);
    ...
    @Input() public set items(value: ReadonlyArray<ViewCableListDtoModel>) {
    ...
    protected deleteCable(id: string) {
        this.delete.emit(id);
    }

    protected editCable(id: string) {
        this.edit.emit(id);
    }
