...
import { masterCableSubListTableColumn } from "@u/constants";
import { ColumnFilterComponent } from "@c/shared/column-filter";
import { FilterColumnsPipe } from "@u/pipe";
import { MatProgressBarModule } from "@angular/material/progress-bar";

import { CableSubListDtoModel } from "./list-cable-sub-table.model";

@Component({
    standalone: true,
    selector: "app-list-cable-sub-table",
    templateUrl: "./list-cable-sub-table.component.html",
    imports: [
        FormDefaultsModule,
        MatTableModule,
        MatSortModule,
...
export class ListCableSubTableComponent {
    ...
    @Input() dataSource: MatTableDataSource<CableSubListDtoModel>;
    @Input() totalLength: number = 0;

    public displayedColumns = [...masterCableSubListTableColumn].map(x => x.key);
    ...
    @Input() public set items(value: ReadonlyArray<CableSubListDtoModel>) {
    ...
    protected deleteCableSub(id: string) {
        this.delete.emit(id);
    }

    protected editCableSub(id: string) {
        this.edit.emit(id);
    }
