...
import { DesignPackageListDtoModel } from "./list-design-package-table.model";
import { masterDesignPackageListTableColumn } from "@u/constants";

@Component({
    standalone: true,
    selector: "app-list-design-package-table",
    templateUrl: "./list-design-package-table.component.html",
    imports: [
        FormDefaultsModule,
        MatTableModule,
        MatSortModule,
...
export class ListDesignPackageTableComponent {
    ...
    @Input() dataSource: MatTableDataSource<DesignPackageListDtoModel>;
    @Input() totalLength: number = 0;

    public displayedColumns = [...masterDesignPackageListTableColumn].map(x => x.key);
    ...
    @Input() public set items(value: ReadonlyArray<DesignPackageListDtoModel>) {
    ...
    protected deleteDesignPackage(id: string) {
        this.delete.emit(id);
    }

    protected editDesignPackage(id: string) {
        this.edit.emit(id);
    }
