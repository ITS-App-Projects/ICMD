...
import { CableCodeListDtoModel } from "./list-cable-code-table.model";
import { masterCableCodeListTableColumn } from "@u/constants";

@Component({
    standalone: true,
    selector: "app-list-cable-code-table",
    templateUrl: "./list-cable-code-table.component.html",
    imports: [
        FormDefaultsModule,
        MatTableModule,
        MatSortModule,
...
export class ListCableCodeTableComponent {
    ...
    @Input() dataSource: MatTableDataSource<CableCodeListDtoModel>;
    @Input() totalLength: number = 0;

    public displayedColumns = [...masterCableCodeListTableColumn].map(x => x.key);
    ...
    @Input() public set items(value: ReadonlyArray<CableCodeListDtoModel>) {
    ...
    protected deleteCableCode(id: string) {
        this.delete.emit(id);
    }

    protected editCableCode(id: string) {
        this.edit.emit(id);
    }
